...
from NewNetwork import Train_Module, initialize_weights
...
def main():
    ...
    # 损失函数和迭代器
    optimizer = AdamW(Train_network.parameters(), lr=args.learning_rate, weight_decay=args.learning_rate*100)

    if args.use_lr_scheduler:
        scheduler = lr_scheduler.MultiStepLR(
            optimizer,
            milestones=[500, 1000, 1500, 3000, 5000, 6000, 8000, 12000],
            gamma=0.9
        )
    ...
    if args.resume_path is not None:
        ...
        pre_epoch = 0
    print('网络模型及优化器构建完成...')
    print("ConvNext-Fusion 共有 {} 个参数".format(sum(x.numel() for x in Train_network.parameters())))
    ...
    writer.add_graph(Train_network, torch.randn((7, in_channel, 256, 256)).to(device))
    print('---------------------------')
    ...
    for epoch in range(args.epochs):
        ...
        for _, image_batch in enumerate(loop):
            ...
            # 复制图像作为标签
            lable = Variable(image_batch.data.clone()).to(device)
            # 正向传播
